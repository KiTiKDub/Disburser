...
void Laf::drawLinearSlider(juce::Graphics& g, int x, int y, int width, int height, float sliderPos, float minSliderPos, float maxSliderPos, const juce::Slider::SliderStyle style, juce::Slider& slider)
{
    ...
    if (slider.isBar())
    {
        ...
        p.startNewSubPath(sliderPos - float(x), (float)height);
        p.lineTo(sliderPos - float(x), float(y));
        ...
        if (sliderPos > width/2)
        {
            auto r = juce::Rectangle<float>(static_cast<float> (x), (float)y + 0.5f, sliderPos - (float)x - 5, 20);
            g.setFont(15);
            r.setLeft((sliderPos - (float)x) - font.getStringWidthFloat(valueStr));

            g.drawFittedText(valueStr, r.toNearestInt(), juce::Justification::Justification::centred, 1);
        }
        else
        {
            auto r = juce::Rectangle<float>(sliderPos + 5, (float)y + 0.5f, font.getStringWidthFloat(valueStr), 20);
            g.setFont(15);
